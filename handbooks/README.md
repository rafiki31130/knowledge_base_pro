...
| `gouvernance-utilisateurs-splunk/` | Livré | Architecte / sysadmin senior Splunk, Splunk Enterprise 9.4.x on-prem, SHC | 8 chapitres : pourquoi gouverner, méthode de projet, quatre axes, findings empiriques 9.4.6, guide RBAC, guide WLM search heads, guide WLM indexers, recommandations chiffrées et glossaire. Disponible en français (canonique) et en anglais ([`gouvernance-utilisateurs-splunk/EN/`](r_knowledge_base_pro/handbooks/gouvernance-utilisateurs-splunk/EN/README.md)). |
| `splunk-shc-knowledge-bundle/` | Livré (erratum 2026-06-18 — `allowSkipReplication` retiré) | Admin / architecte Splunk Enterprise 9.x on-prem, SHC + indexer cluster | 9 chapitres + cheatsheet : foundations (3 bundles), constitution bundle SHC, constitution knowledge bundle, réplication vers peers (classic/cascading/mounted), séquence recherche distribuée, arbre de diagnostic, boîte à outils CLI/REST/logs/SPL, pièges et anti-patterns, cheatsheet diag rapide. Disponible en français (canonique) et en anglais ([`splunk-shc-knowledge-bundle/EN/`](./splunk-shc-knowledge-bundle/EN/README.md)). |
| `splunk-search-performance-handbook/` | Livré (2026-07-26) | Architecte / admin Splunk, Splunk Enterprise 9.x on-prem, IDXC multisite + SHC | 9 chapitres + cheatsheet : anatomie temporelle d'une recherche distribuée (du clic à l'affichage) et leviers d'optimisation par phase — modèle temporel & mesure, parsing SPL, admission/WLM, distribution (bundle/fan-out), map indexeurs, reduce/restitution, accélération, restitution UI + cheatsheet de diagnostic. Disponible en **français** (canonique) et en **anglais** ([`splunk-search-performance-handbook/EN/`](./splunk-search-performance-handbook/EN/README.md)). |
| `splunk-exploitation-handbook/` | Livré (2026-08-03, audit sources 2026-08-04) | Responsable d'exploitation / architecte / admin senior Splunk, Splunk Enterprise 9.x on-prem, plateforme mutualisée | 6 chapitres + cheatsheet : organiser le RUN Splunk en **séparant l'exploitation de l'infrastructure** (socle) de **l'exploitation des données & de l'expérience utilisateur** — modèle à deux plans, catalogue d'activités, **RACI d'exploitation** (six rôles PLT/DAT/SEC/HEB/PO/USR), registre de risques et contraintes opérationnelles, interfaces/contrats/rituels, **confrontation au Splunk Success Framework / CoE Handbook et à des RETEX** + cheatsheet. Français. |
