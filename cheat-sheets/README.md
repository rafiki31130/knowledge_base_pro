...
- [Splunk — détecter les erreurs d'auth en SHC](./splunk-detecter-erreurs-auth-shc.md) — trois recherches pour compter les erreurs après une modification d'`authorize.conf`/`authentication.conf` : par membre et par catégorie, asymétrie entre search heads, impact utilisateur via `_audit` ; grille de lecture bundle fautif vs dérive locale.
